

<div class="contacts">

    <div class="title mt-5">
        Контакты
    </div>
    <div class="subtitle w-lg-50 -2 mb-4">
        Строитенльная компания «Валдайский мастер» работает на строительном рынке более 12 лет. Мы с радостью встретим Вас в одном из наших офисов или проведем встречу по видеосвязи в удаленном формате, без визита к нам в офис.
    </div>


    <!-- Координаты -->

    <div class="row g-5 mt-1 align-items-stretch">
        <div class="col-xl-6">

            <div class="contact-geo h-100">
                <div class="top h-100">
                    <div class="d-flex h-100 flex-column flex-md-row">
                        <div class="col-md-4 order-2 order-md-1 pt-4 pt-md-0">
                            <div class="map h-100">
                                <a target="_blank" href="https://yandex.com/map-widget/v1/?um=constructor%3Afb9cb5b9dabc7ca128b48960bf9ea3dde9f681efbbc37787200d81e5382f8a09&amp;source=constructor"></a>
                                <iframe
                                    src="https://yandex.com/map-widget/v1/?um=constructor%3Afb9cb5b9dabc7ca128b48960bf9ea3dde9f681efbbc37787200d81e5382f8a09&amp;source=constructor"
                                    width="100%" height="100%" frameborder="0">
                                </iframe>
                            </div>
                        </div>
                        <div class="col-md-8 ps-md-4 order-1 order-md-2">

                            <div class="geo-title mt-2 mb-3">
                                Офис в Великом Новгороде
                            </div>
                            <div class="fw-bold">
                                Адрес:
                            </div>
                            <div class="geo-address">
                                Великий Новгород, ул. Державина, выставочная площадка (на напротив АЗС Neste)
                            </div>
                            <div class="my-2">
                                <b>Время работы:</b> ежедневно с 09:00 до 19:00
                            </div>
                            <div class="mb-4">
                                <i>Консультация по предварительной записи.</i>
                            </div>
                            <div class="d-flex flex-column flex-grow-1 justify-content-between">
                                <div class="phones pe-2">
                                    <span class="">
                                        Телефоны в офисе:
                                    </span>
                                    <div class="mt-2">
                                        <nobr><a href="tel:+7 (921) 191-50-01">+7 (921) 191-50-01</a></nobr>
                                    </div>
                                    <div class="">
                                        <nobr><a href="tel:+7 (921) 707-80-10">+7 (921) 707-80-10</a></nobr>
                                    </div>
                                </div>
                                <div class="pt-3">
                                    <a href="">
                                        <img src="./assets/img/raiting_border.png" alt="" class="">
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="border-1 border-top border-style-light-grey my-4 opacity-50"></div>
                <div class="bottom d-flex flex-column flex-md-row">
                    <div class="bottom-left flex-grow-1">
                        <div class="phone"><a href="tel:+7 (800) 550-96-76">+7 (800) 550-96-76</a></div>
                        Звонок по России бесплатный
                    </div>
                    <div class="bottom-right mt-2 mt-md-0">
                        Электронная почта:<br>
                        <a href="mailto:sale@domavn.ru">sale@domavn.ru</a>
                    </div>
                </div>
            </div>

        </div>
        
        <div class="col-xl-6">


        <div class="contact-geo h-100">
                <div class="top h-100">
                    <div class="d-flex h-100 flex-column flex-md-row">
                        <div class="col-md-4 order-2 order-md-1 pt-4 pt-md-0">
                            <div class="map h-100">
                                <iframe src="https://yandex.com/map-widget/v1/?um=constructor%3A807e76c5123199c2d70627cf7ce506746bf874944dbf40f6f1d17242210278bf&amp;source=constructor"
                                    width="100%" height="100%" frameborder="0">
                                </iframe>
                            </div>
                        </div>
                        <div class="col-md-8 ps-md-4 order-1 order-md-2 d-flex flex-column h-100">

                            <div class="geo-title mt-2 mb-3">
                                Офис в Санкт-Петербурге
                            </div>
                            <div class="fw-bold">
                                Адрес:
                            </div>
                            <div class="geo-address">
                                Санкт-Петербург, пр.Стачек, д. 48, корп. 2, БЦ Империал, офис 101А.
                            </div>
                            <div class="my-2">
                                <b>Время работы:</b> ежедневно с 09:00 до 19:00
                            </div>
                            <div class="mb-4">
                                <i>Консультация по предварительной записи.</i>
                            </div>
                            <div class="d-flex flex-column flex-grow-1 justify-content-between">
                                <div class="phones pe-2">
                                    <span class="">
                                        Телефоны в офисе:
                                    </span>
                                    <div class="mt-2">
                                        <nobr><a href="tel:+7 (921) 191-50-01">+7 (921) 191-50-01</a></nobr>
                                    </div>
                                    <div class="">
                                        <nobr><a href="tel:+7 (921) 707-80-10">+7 (921) 707-80-10</a></nobr>
                                    </div>
                                </div>
                                <div class="pt-3">
                                    <a href="">
                                        <img src="./assets/img/raiting_border.png" alt="" class="">
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="border-1 border-top border-style-light-grey my-4 opacity-50"></div>
                <div class="bottom d-flex flex-column flex-md-row">
                    <div class="bottom-left flex-grow-1">
                        <div class="phone"><a href="tel:+7 (800) 550-96-76">+7 (800) 550-96-76</a></div>
                        Звонок по России бесплатный
                    </div>
                    <div class="bottom-right mt-2 mt-md-0">
                        Электронная почта:<br>
                        <a href="mailto:sale@domavn.ru">sale@domavn.ru</a>
                    </div>
                </div>
            </div>

        </div>

    </div>


    <!-- / Координаты -->

    
    <!-- Баннер -->

    <div class="contact-geo contact-banner my-5">

        <div class="banner-description order-1">

            <div class="top h-100">
                <div class="d-flex h-100 flex-column flex-md-row">
                    <div class="col-lg-4">
                        <div class="map h-100">
                            <img src="./assets/img/photo/ustugna.png" alt="">
                        </div>
                    </div>
                    <div class="col-lg-8 ps-md-4 order-1 order-md-2">

                        <div class="geo-title mt-2 mb-3">
                            Производство в Устюжне
                        </div>
                        <div class="fw-bold">
                            Адрес:
                        </div>
                        <div class="geo-address">
                            Вологодская область, г. Устюжна, производственная база <b>«Валдайский мастер»</b>
                        </div>
                        <div class="my-2">
                            <b>Время работы:</b> ежедневно с 09:00 до 19:00
                        </div>
                        <div class="mb-4">
                            <i>Экскурсия на производство по предварительной записи.</i>
                        </div>
                    </div>
                </div>
            </div>

        </div>
                
        <div class="banner-contacts order-3">

            <div class="d-flex flex-column h-100 w-100 justify-content-end">
                <div class="border-1 border-top border-style-light-grey my-4 opacity-50"></div>
                <div class="bottom d-flex flex-column flex-md-row">
                    <div class="bottom-left flex-grow-1">
                        <div class="phone"><a href="tel:+7 (800) 550-96-76">+7 (800) 550-96-76</a></div>
                        Звонок по России бесплатный
                    </div>
                    <div class="bottom-right mt-2 mt-md-0">
                        Электронная почта:<br>
                        <a href="mailto:sale@domavn.ru">sale@domavn.ru</a>
                    </div>
                </div>
            </div>


        </div>

        <div class="banner-gallery ps-lg-5 pt-4 pt-lg-0 order-2">
            <!-- Слайдер - Галерея -->
            <div class="slider-gallery">
                <div class="slider-gallery-items">
                    <div>
                        <div class="wrapper"><img src="./assets/img/photo/house01.png" alt=""></div>
                    </div>
                    <div>
                        <div class="wrapper"><img src="./assets/img/photo/house01.png" alt=""></div>
                    </div>
                    <div>
                        <div class="wrapper"><img src="./assets/img/photo/article_house_1.jpg" alt=""></div>
                    </div>
                    <div>
                        <div class="wrapper"><img src="./assets/img/photo/article_house_1.jpg" alt=""></div>
                    </div>
                </div>
                <div class="slider-gallery-nav pt-2"></div>
            </div>
            <!-- / Слайдер - Галерея -->
        </div>


    </div>

    <!-- / Баннер -->
    
    <!-- Баннер -->

    <div class="contact-geo contact-banner my-5">

<div class="banner-description order-1">

    <div class="top h-100">
        <div class="d-flex h-100 flex-column flex-md-row">
            <div class="">

                <div class="geo-title mt-2 mb-3">
                    Приглашаем на построеные и строящиеся объекты в Москве и Ленинградской области
                </div>
                <div class="theme-button gradient outline padding-2 d-block d-md-inline-block mb-4 mb-md-5">
                    <a href="">
                        <div class="wrap bg-style-lightest text-center">
                            Записаться на экскурсию
                        </div>
                    </a>
                </div>
                <div class="my-2">
                    <b>Время работы:</b> ежедневно с 09:00 до 19:00
                </div>
                <div class="mb-4">
                    <i>Посещение объектов строительсва по предварительной записи.</i>
                </div>
            </div>
        </div>
    </div>

</div>
        
<div class="banner-contacts order-3">

    <div class="d-flex flex-column h-100 w-100 justify-content-end">
        <div class="border-1 border-top border-style-light-grey my-4 opacity-50"></div>
        <div class="bottom d-flex flex-column flex-md-row">
            <div class="bottom-left flex-grow-1">
                <div class="phone"><a href="tel:+7 (800) 550-96-76">+7 (800) 550-96-76</a></div>
                Звонок по России бесплатный
            </div>
            <div class="bottom-right mt-2 mt-md-0">
                Электронная почта:<br>
                <a href="mailto:sale@domavn.ru">sale@domavn.ru</a>
            </div>
        </div>
    </div>


</div>

<div class="banner-gallery ps-lg-5 pt-4 pt-lg-0 order-2">
    <!-- Слайдер - Галерея -->
    <div class="slider-gallery">
        <div class="slider-gallery-items">
            <div>
                <div class="wrapper"><img src="./assets/img/photo/house01.png" alt=""></div>
            </div>
            <div>
                <div class="wrapper"><img src="./assets/img/photo/house01.png" alt=""></div>
            </div>
            <div>
                <div class="wrapper"><img src="./assets/img/photo/article_house_1.jpg" alt=""></div>
            </div>
            <div>
                <div class="wrapper"><img src="./assets/img/photo/article_house_1.jpg" alt=""></div>
            </div>
        </div>
        <div class="slider-gallery-nav pt-2"></div>
    </div>
    <!-- / Слайдер - Галерея -->
</div>


</div>

<!-- / Баннер -->

</div>




    
<!-- Консультация -->

<div class="border-1 border-top border-style-primary-green mt-4 mt-lg-5 d-block d-lg-none"></div>

<div class="my-3 my-lg-5">
    <?php include "./components/get-consult.php"; ?>
</div>

<div class="border-1 border-top border-style-primary-green my-3 my-lg-5 mb-5 d-block d-lg-none"></div>

<!-- Консультация -->
